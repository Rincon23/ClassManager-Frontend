...
export default function AtualizarAulaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/class`, {
        ...
        body: JSON.stringify({ oldDate, oldTimeSlot, newDate, newTimeSlot }),
      });

      const data = await res.json();
      if (!res.ok) {
        setMensagem(data.message || "Erro ao atualizar aula.");
        return;
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#0f172a] text-white px-6">
      <h1 className="text-4xl font-bold mb-10 text-indigo-400">Atualizar Aula</h1>

      <form
        onSubmit={handleSubmit}
        className="bg-[#192747] p-10 rounded-2xl shadow-lg w-96 border-2 border-indigo-500"
      >
        <label className="block text-gray-300 mb-2 font-semibold">Data Atual</label>
        <input
          type="date"
          value={oldDate}
        ...
        <label className="block text-gray-300 mb-2 font-semibold">Horário Atual</label>
        <select
          value={oldTimeSlot}
          onChange={(e) => setOldTimeSlot(e.target.value)}
          className="w-full p-2 rounded bg-[#0f172a] border border-gray-600 text-white mb-4"
          required
        >
        ...
        <label className="block text-gray-300 mb-2 font-semibold">Nova Data</label>
        <input
          type="date"
          value={newDate}
        ...
        <label className="block text-gray-300 mb-2 font-semibold">Novo Horário</label>
        <select
          value={newTimeSlot}
